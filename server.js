...
app.delete('/api/links/:id',auth,(req,res)=>{const r=db.prepare('DELETE FROM links WHERE id=? AND user_id=?').run(req.params.id,req.user.id);if(!r.changes)return res.status(404).json({error:'Ссылка не найдена'});res.json({ok:true})});
